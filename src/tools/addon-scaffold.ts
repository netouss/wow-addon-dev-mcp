...
export class AddonScaffold {
  ...
  private generateConfigLua(addonName: string, dbName: string): string {
    return [
      `local ADDON_NAME, ns = ...`,
      ``,
      `-- Options panel using the modern Settings API (Retail 10.0+).`,
      `-- Defer registration until after our SavedVariables exist.`,
      ...
      `        category,`,
      `        "${addonName}_Enabled",   -- unique variable id`,
      `        "enabled",                -- key inside the saved table`,
      `        ${dbName},                -- target table`,
